...
jQuery(document).ready(function () {
    var modal = $('#modalPengelola');
    
    oTable = $('#list-pengelola').DataTable({
        processing: true,
        serverSide: true,
        ajax: laroute.route('admin.pengelola'),
        columns: [{
                data: 'DT_RowIndex',
                name: 'DT_RowIndex'
            },
    ...
    $(document).on('click', '#btn_tambah', function () {
        $('#form-pengelola')[0].reset();
        modal.find('input#metode').val('tambah');
        $('#modal_title').text('Tambah pengelola Baru');
    ...
    $(document).on('click', '.btn-edit', function () { 
        ...
        $.ajax({
            url: laroute.route('admin.pengelola.edit', { id: id }),
            type: "GET",
            dataType: "JSON",
            beforeSend: function(){
                Swal.fire({
    ...
    $(document).on('click', '.btn-hapus', function () { 
        ...
        .then((result) => {
            if (result.value) {
            $.ajax({
                url: laroute.route('admin.pengelola.hapus', { id: id }),
                type: "GET",
                dataType: "JSON",
                beforeSend: function(){
                    Swal.fire({
    ...
    $("#form-pengelola").validate({
        onfocusout: function(element) {
            $(element).valid()
            if ($(element).valid()) {
                $('#form-pengelola').find('button:submit').prop('disabled', false);  
            } else {
                $('#form-pengelola').find('button:submit').prop('disabled', 'disabled');
            }
        },    
        ...
        messages: {
            ...
            jabatan: {
                required: "Jabatan pengelola Wajib Diisi!",
            },
        },
        submitHandler: function (form) {
            var fomr = $('form#form-pengelola')[0];
            var formData = new FormData(fomr);
            if($('#metode').val() == 'tambah')
            {
                url = laroute.route('admin.pengelola.simpan');
            }else{
                url = laroute.route('admin.pengelola.update');
            }
            $.ajax({
                type: 'POST',
                url: url,
