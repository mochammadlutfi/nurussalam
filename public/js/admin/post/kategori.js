...
jQuery(function() {
    ...
    a.validate({
        onfocusout: function(element) {
            $(element).valid()
            if ($(element).valid()) {
                $('#form-kategori').find('button:submit').prop('disabled', false);  
            } else {
                $('#form-kategori').find('button:submit').prop('disabled', 'disabled');
            }
        },    
        ...
        submitHandler: function (form) {
            ...
            {
                url = laroute.route('admin.postKategori.simpan');
            }else{
                url = laroute.route('admin.postKategori.update');
            }
            $.ajax({
                type: 'POST',
                url: url,
    ...
    $('#list-kategori').DataTable({
        ...
        pagingType: "simple",
        pageLength : 10,
        ajax: laroute.route('admin.postKategori'),
        columns: [
            {
                data: 'nama',
                name: 'nama'
...
function edit(id){
    ...
    // alert(id);
    $.ajax({
        url : laroute.route('admin.postKategori.edit', {id : id}),
        type: "GET",
        dataType: "JSON",
        success: function(response)
        {
           ...
           $('#form-kategori').find('input[name="id"]').val(response.kategori_id);
...
function hapus(id) {
    ...
    .then((result) => {
        if (result.value) {
        $.ajax({
            url: laroute.route('admin.postKategori.hapus', { id: id }),
            type: "GET",
            dataType: "JSON",
            beforeSend: function(){
                Swal.fire({
